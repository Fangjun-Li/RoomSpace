...
def select_wall(openings: List[Tuple[int, int]], boundary_groups: BoundaryGroups):
    chosen_openings = dict()
    for opening in openings:
        candidates = list(boundary_groups[opening])
        # candidates [((0.0, 5.517), (5.517, 5.517)), ((0.0, 0.0), (5.517, 0.0)), ((5.517, 0.0), (5.517, 5.517)), ((0.0, 0.0), (0.0, 5.517))]
        # candidates[0]: north    [1]:south     [2]: east     [3]west
        # Find the wall with the smallest y coordinate (assuming this is the north wall)
        north_wall_candidates = [c for c in candidates if c[0][0] == 0 and c[0][1] != 0 and c[1][0] != 0 and c[1][1] != 0]  
        south_wall_candidates = [c for c in candidates if c[0][0] == 0 and c[0][1] == 0 and c[1][0] != 0 and c[1][1] == 0]  
        east_wall_candidates = [c for c in candidates if c[0][0] != 0 and c[0][1] == 0 and c[1][0] != 0 and c[1][1] != 0]        
        west_wall_candidates = [c for c in candidates if c[0][0] == 0 and c[0][1] == 0 and c[1][0] == 0 and c[1][1] != 0]  
        north_wall = south_wall_candidates[0]  
        #print(north_wall)

        chosen_openings[opening] = north_wall
    return chosen_openings
...
def add_door_meta(
    ...
    for orig_boundary, wall in door_walls.items():
        ...
        door_width = door["xSize" if use_frame else "openXSize"].iloc[0]
        start_door_position = (wall_size - door_width) / 2
        start_door_position = round(start_door_position / 0.25) * 0.25

        # set the position of the door in the middle of the wall
        # NOTE: z is currently ignored on the Unity side
        bounding_box = BoundingBox(
            min=Vector3(x=start_door_position, y=0, z=0),
            max=Vector3(
                x=start_door_position + door_width,
                y=door["ySize"].iloc[0],
                z=0,
            ),
        )
